...
import { RecruiterCandidate, recruiterApi } from '../../api/recruiterApi';
import {
  ShieldCheck,
  MapPin,
  Flame,
...
export const CandidateDossierModal: React.FC<CandidateDossierModalProps> = ({
  ...
  const isContactHidden = privacy?.contactVisibility === 'hidden';

  const handleDownload = async () => {
    if (isContactHidden) {
      setDownloadError('This candidate has set their resume and contact details to hidden.');
      return;
    }
    try {
      setIsDownloading(true);
      setDownloadError(null);
      await recruiterApi.downloadCandidateResume(candidate.id, candidate.name);
    } catch (err: any) {
      setDownloadError(err.message || 'Unable to download candidate resume.');
    } finally {
      ...
      <div className="space-y-5 -mt-4 text-xs text-[#38434F]">
        ...
        <div className="p-4 rounded-xl bg-[#F3F6F8] border border-[#E8E8E8] space-y-2.5">
          <h4 className="font-bold text-[#1D2226] uppercase font-mono text-[11px] tracking-wider flex items-center gap-2">
            ...
            Verified Production Repositories ({candidate.projectsCount ?? 1})
          </h4>
          <div className="space-y-2">
            {(candidate.featuredProjects || []).map((pName: string) => (
              <div
                key={pName}
        ...
        {/* ==================== 5. ATTACHED RESUME ==================== */}
        <div className="p-4 rounded-xl bg-[#F3F6F8] border border-[#E8E8E8] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 font-mono text-xs">
            ...
            <div>
              ...
              <span className="text-[10px] text-[#788896] block">
                {isContactHidden ? '🔒 Direct Resume Restricted by Candidate' : `ATS Score: ${candidate.atsScore}% • Verified`}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 w-full sm:w-auto">
            {downloadError && (
              <span className="text-[10px] text-rose-600 font-mono">{downloadError}</span>
            )}
            <Button
              size="xs"
              variant={isContactHidden ? 'secondary' : 'outline'}
              disabled={isContactHidden || isDownloading}
              onClick={handleDownload}
              icon={isContactHidden ? <Lock className="w-3 h-3" /> : <Download className="w-3 h-3" />}
            >
              {isDownloading ? 'Downloading...' : isContactHidden ? 'Resume Hidden' : 'Download Resume'}
            </Button>
          </div>
        </div>
      </div>
