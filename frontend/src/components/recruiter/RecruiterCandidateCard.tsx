...
import { RecruiterCandidate } from '../../api/recruiterApi';
import {
  ShieldCheck,
  MapPin,
  Flame,
...
export const RecruiterCandidateCard: React.FC<RecruiterCandidateCardProps> = ({
  ...
  const searchStatus = privacy?.searchStatus || 'actively_looking';
  const searchStatusBadges: Record<string, { label: string; color: string }> = {
  ...
  const statusConfig = searchStatusBadges[searchStatus] || searchStatusBadges.actively_looking;
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono text-[11px] pt-1">
          ...
          <div className="p-2.5 rounded-xl bg-[#F3F6F8] border border-[#E8E8E8] space-y-0.5">
            ...
            <span className="font-bold text-[#1D2226] block">{candidate.projectsCount ?? 1} Repositories</span>
            <span className="text-[9px] text-sky-600 truncate block">
              {candidate.featuredProjects?.[0] || 'Full Stack App'}
            </span>
          </div>
