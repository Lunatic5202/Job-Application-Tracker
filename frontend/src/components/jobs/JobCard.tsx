...
import { JobItem, ApplicationStatus } from '../../types';
import {
  Building2,
  MapPin,
  Clock,
...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  // Format dynamic status badge/button
  const effectiveStatus = applicationStatus || (isApplied ? 'Applied' : null);
  const statusLabel =
  ...
  const statusStyle =
    effectiveStatus === 'Offer'
      ...
      : effectiveStatus === 'Interview'
      ? 'text-[#0A66C2] border-[#0A66C2]/40 bg-[#E8F3FF] hover:bg-[#d0e6fc]'
      : effectiveStatus === 'Rejected'
  ...
  return (
    <Card className="p-4 flex flex-col justify-between hover:border-[#0A66C2]/40 hover:shadow-md transition-all duration-150 space-y-3.5 group bg-white border border-[#D9D9D9] shadow-sm">
      ...
      {/* Bottom Action Buttons: [View Job], [Apply/Status], [Analyze Match] */}
      <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-[#E8E8E8]">
        ...
        <Button
          size="xs"
          variant={effectiveStatus ? 'outline' : 'primary'}
          onClick={() => onApply(job)}
          className={`text-[11px] px-1.5 font-semibold ${statusStyle}`}
          icon={effectiveStatus ? <CheckCircle2 className="w-3 h-3" /> : <Send className="w-3 h-3" />}
          title={effectiveStatus ? `Application stage: ${effectiveStatus}. Click to manage/reapply.` : 'Apply to this position'}
        >
          {statusLabel}
        </Button>
