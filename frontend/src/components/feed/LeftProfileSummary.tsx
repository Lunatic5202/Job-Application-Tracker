...
export const LeftProfileSummary: React.FC<LeftProfileSummaryProps> = ({
  ...
  const authorName = user?.name || (user?.email ? user.email.split('@')[0] : 'Engineer');
  const authorInitials = user?.name
    ? user.name.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()
    : (user?.email ? user.email[0].toUpperCase() : 'CX');
  const authorHeadline = user?.role === 'recruiter' ? 'Technical Recruiter' : (user?.headline || 'Software Engineer');
  const authorLocation = user?.location || 'Remote';
  const atsScore = user?.atsScore ? `${user.atsScore}%` : 'Active';
      ...
      <Card className="p-4 bg-white border border-[#D9D9D9] space-y-3.5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <h3 className="text-sm font-bold text-[#1D2226] truncate">{authorName}</h3>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#E8E8E8] text-[11px] font-mono">
          <div className="p-2 rounded-lg bg-[#F3F6F8] border border-[#E8E8E8]">
            <span className="text-[9px] uppercase text-[#788896] block">Status</span>
          ...
          <div className="p-2 rounded-lg bg-[#F3F6F8] border border-[#E8E8E8]">
            <span className="text-[9px] uppercase text-[#788896] block">ATS Rating</span>
            <span className="font-bold text-[#0A66C2]">{atsScore}</span>
          </div>
        </div>
