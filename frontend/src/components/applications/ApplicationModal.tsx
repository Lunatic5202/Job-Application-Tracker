...
export const ApplicationModal: React.FC<ApplicationModalProps> = ({
  ...
  useEffect(() => {
    let isMounted = true;
    resumeApi.listResumes().then((list) => {
  ...
  useEffect(() => {
    if (initialData) {
      reset({
        ...
        resume: initialData.resume || (resumes[0]?.filename || 'Primary_Resume.pdf'),
      });
    } else {
      reset({
        company: '',
        ...
        priority: 'High',
        notes: '',
        resume: resumes[0]?.filename || resumes[0]?.name || 'Primary_Resume.pdf',
      });
    }
  }, [initialData, reset, isOpen, resumes]);
                ...
                {resumes.length > 0 ? (
                  resumes.map((r) => (
                    <option key={r.id} value={r.filename || r.name || r.id}>
                      {r.filename || r.name} {r.atsScore ? `(ATS ${r.atsScore}%)` : ''}
                    </option>
                  ))
                ) : (
                  <option value="Primary_Resume.pdf">Primary_Resume.pdf (Active)</option>
                )}
              </select>
