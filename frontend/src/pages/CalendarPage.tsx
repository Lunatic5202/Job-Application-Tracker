...
export const CalendarPage: React.FC = () => {
  ...
  // Current calendar month state (defaults to current date)
  ...
  const handleSyncGoogleCalendar = async () => {
    ...
    try {
      ...
      setSyncStatus(res);
      setSyncNotice(`✅ Synced ${res.syncedCount} events with Google Calendar${res.accountEmail ? ` (${res.accountEmail})` : ''}`);
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      const created = await calendarSyncApi.createEvent({
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#56687A]">
              Synced with <strong className="text-[#1D2226]">{syncStatus?.accountEmail || 'Google Calendar'}</strong> • Last updated {syncStatus?.lastSyncedAt || 'Recently'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredEvents.length === 0 ? (
            <div className="col-span-full py-8 text-center text-xs text-slate-500 font-mono">
              No calendar milestones scheduled. Click &quot;Schedule Milestone&quot; to add your first event.
            </div>
          ) : (
