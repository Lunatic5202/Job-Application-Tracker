...
export const ProgressPage: React.FC = () => {
  ...
  const [skillsData, setSkillsData] = useState<SkillTrajectory[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let isMounted = true;
    Promise.all([
      progressApi.getProgressOverview(),
      progressApi.getActivityHistory(activityTimeframe),
      progressApi.getSkillTrajectories(),
    ])
      .then(([ov, act, sk]) => {
        if (!isMounted) return;
        setOverview(ov);
  ...
  const currentAts = overview?.currentAtsScore ?? user?.atsScore ?? 0;
  const resumeGrowthData = [
    { version: 'v1.0', score: Math.max(0, currentAts - 20) },
    { version: 'v1.1', score: Math.max(0, currentAts - 12) },
    { version: 'v1.2', score: Math.max(0, currentAts - 6) },
    { version: 'Current', score: currentAts },
  ];
      ...
      <PageHeader
        ...
        badge={
          <Badge variant="brand" size="sm">
            Verified Profile: {user?.name || user?.email || 'Engineering Candidate'}
          </Badge>
        }
        actions={
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-[10px] text-[#788896] font-mono">/ {overview?.totalQuestions ?? 0}</span>
          </div>
          <span className="text-[10px] text-emerald-700 font-semibold font-mono">Verified Activity</span>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          ...
          <span className="text-[10px] text-[#8A6100] font-semibold font-mono">Continuous</span>
        </div>

        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          <span className="text-[10px] uppercase font-mono text-[#788896]">First-Submit Acc</span>
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-bold text-emerald-700 font-mono">{overview?.accuracy ?? 0}%</span>
          </div>
          <span className="text-[10px] text-emerald-700 font-semibold font-mono">Evaluated Tests</span>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          ...
          <span className="text-[10px] text-[#0A66C2] font-semibold font-mono">Live Resume</span>
        </div>

        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          <span className="text-[10px] uppercase font-mono text-[#788896]">Projects Added</span>
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-bold text-[#1D2226] font-mono">{overview?.projectsCompleted ?? 0}</span>
            <span className="text-[10px] text-[#788896] font-mono">Live</span>
          </div>
          <span className="text-[10px] text-[#0A66C2] font-semibold font-mono">Portfolio</span>
        </div>

        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-1 shadow-xs">
          <span className="text-[10px] uppercase font-mono text-[#788896]">Certifications</span>
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-bold text-[#1D2226] font-mono">{overview?.certificationsCount ?? 0}</span>
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
          </div>
          <span className="text-[10px] text-emerald-700 font-semibold font-mono">Verified</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <Card className="lg:col-span-7 flex flex-col bg-white border border-[#D9D9D9] shadow-xs">
          ...
          <CardContent className="p-4 flex-1">
            {activityData.length === 0 ? (
              <div className="h-64 flex flex-col items-center justify-center text-center p-6 border border-dashed border-[#D9D9D9] rounded-xl bg-[#F3F6F8]">
                <Clock className="w-6 h-6 text-[#788896] mb-2" />
                <p className="text-xs font-semibold text-[#1D2226]">No practice activity recorded yet</p>
                <p className="text-[11px] text-[#56687A] mt-1">Start coding practice or learning modules to track your daily study hours.</p>
              </div>
            ) : (
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={activityData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="period" stroke="#788896" fontSize={11} tickLine={false} />
                    <YAxis stroke="#788896" fontSize={11} tickLine={false} />
                    <Tooltip contentStyle={{ backgroundColor: '#FFFFFF', borderColor: '#D9D9D9', borderRadius: '8px', fontSize: '11px', color: '#1D2226' }} />
                    <Legend wrapperStyle={{ fontSize: '11px', paddingTop: '8px' }} />
                    <Bar dataKey="questionsSolved" name="Questions Solved" fill="#0A66C2" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="studyHours" name="Study Hours" fill="#137333" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <Card className="lg:col-span-5 flex flex-col bg-white border border-[#D9D9D9] shadow-xs">
          ...
          <CardContent className="p-4 flex-1">
            {skillsData.length === 0 ? (
              <div className="h-64 flex flex-col items-center justify-center text-center p-6 border border-dashed border-[#D9D9D9] rounded-xl bg-[#F3F6F8]">
                <TrendingUp className="w-6 h-6 text-[#788896] mb-2" />
                <p className="text-xs font-semibold text-[#1D2226]">No skill trajectory data recorded yet</p>
                <p className="text-[11px] text-[#56687A] mt-1">Practice verified skills or take technical assessments to track growth.</p>
              </div>
            ) : (
              <div className="h-64 w-full overflow-y-auto space-y-3 p-1">
                {skillsData.map((sk) => (
                  <div key={sk.name} className="p-3 rounded-xl bg-[#F3F6F8] border border-[#E8E8E8] space-y-1.5">
                    <div className="flex items-center justify-between text-xs font-semibold text-[#1D2226]">
                      <span>{sk.name}</span>
                      <span className="text-[#0A66C2] font-mono">{sk.currentScore}% (+{sk.growthPercentage}%)</span>
                    </div>
                    <div className="w-full h-2 bg-white rounded-full overflow-hidden border border-[#D9D9D9]">
                      <div className="h-full bg-[#0A66C2] rounded-full transition-all duration-500" style={{ width: `${sk.currentScore}%` }} />
                    </div>
                  </div>
                ))}
              </div>
