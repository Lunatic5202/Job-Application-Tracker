...
export const RecruiterPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      recruiterApi.getRecruiterMetrics(),
    ])
      .then(([candList, metricsData]) => {
        if (!isMounted) return;
  ...
  const handleShortlistToggle = async (candidateId: string) => {
    try {
      const res = await recruiterApi.toggleShortlistCandidate(candidateId);
      setCandidates((prev) =>
        prev.map((c) => (c.id === candidateId ? { ...c, isShortlisted: res.isShortlisted } : c))
      );
      if (selectedCandidate && selectedCandidate.id === candidateId) {
        setSelectedCandidate((prev) => (prev ? { ...prev, isShortlisted: res.isShortlisted } : null));
      }
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-0.5 shadow-xs">
          ...
          <p className="text-xl font-bold text-[#1D2226] font-mono">{metrics?.jobsPosted ?? 0}</p>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-0.5 shadow-xs">
          ...
          <p className="text-xl font-bold text-[#1D2226] font-mono">{metrics?.applicationsCount ?? 0}</p>
          <span className="text-[10px] text-sky-600 font-mono font-semibold">Total Submissions</span>
        </div>
        ...
        >
          ...
          <p className="text-xl font-bold text-[#8A6100] font-mono">{metrics?.shortlistedCount ?? shortlistedCount}</p>
        ...
        >
          ...
          <p className="text-xl font-bold text-emerald-600 font-mono">{metrics?.interviewsCount ?? interviewCount}</p>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-[#D9D9D9] space-y-0.5 col-span-2 sm:col-span-1 shadow-xs">
          ...
          <p className="text-xl font-bold text-[#1D2226] font-mono">{metrics?.hiredCount ?? 0}</p>
          <span className="text-[10px] text-[#788896] font-mono">Targets Met</span>
        </div>
      </div>
