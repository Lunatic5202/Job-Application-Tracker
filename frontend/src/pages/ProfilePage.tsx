...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    resumeApi.getActiveResume().then((r) => {
      if (isMounted && r) {
        setActiveResume(r);
      }
    }).catch(() => {});
      ...
      <Card className="p-6 bg-white border border-[#D9D9D9] space-y-5 shadow-sm">
        ...
        <div className="pt-4 border-t border-[#E8E8E8] flex items-center justify-between flex-wrap gap-3 text-xs">
          ...
          >
            ...
            <span>{activeResume ? (activeResume.filename || activeResume.name) : 'Upload Resume'}</span>
          ...
          <Card className="p-4 bg-white border border-[#D9D9D9] space-y-3 shadow-sm">
            <div className="flex items-center justify-between pb-2 border-b border-[#E8E8E8]">
              ...
              {activeResume?.atsScore ? (
                <Badge variant="success" size="sm">
                  {activeResume.atsScore}% ATS Score
                </Badge>
              ) : null}
            </div>
            ...
            {activeResume ? (
              <div className="p-3 rounded-xl bg-[#F3F6F8] border border-[#E8E8E8] space-y-2">
                ...
                <p className="text-[11px] text-[#56687A]">
                  Verified candidate document indexed for recruiter screening and ATS analysis.
                </p>
                <div className="flex items-center justify-between text-[10px] font-mono text-[#788896] pt-1">
                  <span>{activeResume.format || 'PDF'} • {activeResume.size || '1.5 MB'}</span>
                  <span>{activeResume.uploadDate || 'Active'}</span>
                </div>
              </div>
            ) : (
              <div className="p-3.5 rounded-xl bg-[#F3F6F8] border border-dashed border-[#D9D9D9] text-center space-y-1">
                <p className="text-xs font-semibold text-[#1D2226]">No resume uploaded</p>
                <p className="text-[11px] text-[#56687A]">Upload your resume in Resume AI to get evaluated.</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <Link to="/resume" className="w-full">
                <Button size="xs" variant="outline" className="w-full text-[11px]">
                  {activeResume ? 'Inspect in ATS' : 'Upload Resume'}
                </Button>
              </Link>
              {activeResume ? (
                <a
                  href={`http://localhost:8000/api/resumes/${activeResume.id}/download`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full"
                >
