...
export const CodingPracticePage: React.FC = () => {
  ...
  const handleAiAction = async (action: AiActionType) => {
    ...
    try {
      let res;
      let badge = 'AI Copilot';
      if (action === 'hint') {
        badge = 'Key Idea';
        res = await aiApi.getCodingHint(currentProblem.id, code, selectedLanguage);
      } else if (action === 'explain_error') {
        badge = 'Diagnostic';
        const errorText = executionResult?.stderr || executionResult?.stdout || 'Syntax or runtime error';
        res = await aiApi.explainError(code, errorText, selectedLanguage);
      ...
      const errMsg = err?.response?.data?.detail || err?.message || 'AI assistant is currently unavailable.';
      setAiResponse({
        action,
        title: 'AI Service Notice',
        badge: 'Notice',
        content: `⚠️ **Unable to complete AI request**:\n\n${errMsg}\n\nPlease verify backend service status and Groq API credentials.`,
      });
    } finally {
